...
# --------------------------------------------------------------------------
# DATABASE-INITIALISERING (KJØRES BARE ÉN GANG PER APPSTART - IKKE VED HVERT KLIKK)
# --------------------------------------------------------------------------
@st.cache_resource
...
st.set_page_config(page_title="ScanLager", page_icon="🛠️", layout="wide")
...
if not st.session_state.aktiv_bedrift_id or not st.session_state.bruker:
    ...
    if st.session_state.vis_bedrift_registrering:
        st.title("🛠️ ScanLager — Registrer bedrift")
        ...
        st.title("🛠️ ScanLager — Registrering")
...
# --- 2. SKANN OG OCR (nå med pytesseract i stedet for easyocr) ---
    ...
    if valg_metode.startswith("📌"):
        ...
        if HAR_QR_MODUL:
            ...
            st.error("QR-modul ikke installert. Legg til 'streamlit-qrcode-scanner' i requirements.txt.")
        ...
        if HAR_OCR_MODUL:
            ...
            if kamera_bilde:
                with st.spinner("Analyserer bilde og nullstiller kamera..."):
                    ...
                    # Gjør bildet gråskala + litt kontrastforsterkning - forbedrer treffsikkerheten til tesseract merkbart
            ...
            st.error("OCR-modul ikke installert. Legg til 'pytesseract' i requirements.txt og 'tesseract-ocr' + 'tesseract-ocr-nor' i packages.txt.")
